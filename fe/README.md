...
1. **Đăng nhập**: lấy JWT từ auth-service và `customerId` từ booking-service.
2. **Các request sau**: gắn `Authorization: Bearer <token>` vào header.
3. **Token hết hạn**: interceptor tự động gọi `/api/v1/auth/refresh` và thử lại request.
